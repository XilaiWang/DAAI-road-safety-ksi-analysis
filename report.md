...
- **Stakeholders.** These span road‑safety partnerships and local‑authority highway teams (own budgets/review queues), highway engineers and transport planners (deliver schemes), motor/fleet insurers and risk managers (for territorial risk awareness and loss‑prevention, not pricing), and budget / partnership holders — at the interface of public‑safety allocation and commercial road‑risk management.
...
- **Why suitable.** It is large, genuinely collected (single‑feature AUCs of 0.50–0.55 rule out synthetic separation), and records pre‑existing road context — speed, road type, urban/rural, lighting, weather, junction, time — needed to characterise severity.
...
- **External data — what, why, how.** Three external sources were joined: English Indices of Deprivation 2019 (by authority name), DfT vehicle‑miles (by ONS code via a district‑to‑county lookup), and official multi‑year STATS19 (2015–2024). They measure equity and convert raw counts into exposure‑adjusted burden; coverage was a transparent waterfall (88.3% England‑matched).
...
- **Methods.** Logistic regression (transparent baseline), a decision tree, k‑NN, a random forest and XGBoost were compared for interpretability and a strong tabular benchmark.
...
- **Interpretation and equity.** Probabilities were calibrated; a cost‑based threshold reflected the higher cost of a missed KSI; SHAP explained drivers; k‑means and hierarchical clustering cross‑checked segments; and a cluster‑robust nested logistic regression with a vehicle‑mile exposure denominator addressed equity.
...
- **Operating point.** At a 5:1 false‑negative‑to‑false‑positive cost the model recalls ~40% of KSI contexts (precision ~20%), rising to ~91% at 10:1, so the threshold is a transparent budget lever.
...
- **Actions.** Public bodies should use it as a triage signal (not an automatic rule): prioritise high‑speed rural contexts for engineering, speed and lighting review and deprived / high‑exposure urban areas for traffic‑calming and vulnerable‑user protection, with the threshold set to review capacity. Insurers should use the same ranking for loss‑prevention and fleet‑risk guidance, reserving awareness and partnership targeting — not pricing. Illustratively, screening 10,000 contexts could surface ~580 KSI‑risk cases and, at a 5% effect, avoid ~29 KSI outcomes.
- **Limitations.** The data are observational (associations, not causal effects), exposure covers motor vehicles only; deprivation is area‑level (ecological‑fallacy risk, reduced but not eliminated by an LSOA check); and performance suits prioritisation, not precise prediction.
- **Future enhancements.** Adding road‑geometry, traffic‑flow and pedestrian/cyclist‑exposure features, evaluating top‑k targeting, and pairing the framework with intervention‑rollout data for quasi‑experimental causal estimates would strengthen it.
