...
- **Methods.** This is a **binary classification** problem: dependent variable `KSI` (1 = killed/seriously‑injured, 0 = slight); independent variables are at‑scene road/environment/time fields. A binary outcome with mixed categorical/numeric predictors makes **logistic regression** (interpretable — and, unlike a linear probability model, it bounds predicted probabilities to [0,1] and allows a non‑constant marginal effect) and **XGBoost** (strong benchmark) natural; a decision tree, k‑NN and random forest are compared. XGBoost was tuned by randomised search on 2021 (gain +0.002), and k‑means segments added as predictors gave no lift.
